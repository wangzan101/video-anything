...
根据是否有**人工字幕**决定是否跳过 ASR:
- 若第 1 步已产出**人工字幕** `*.vtt`(文件名不含 `.auto.` 或 `-auto.`)，且用户只要文案 → 直接读 `.vtt`,清洗成纯文本即可,**跳过 ASR**(更快更准)。判别逻辑见 `scripts/lib/asr_utils.py::is_manual_subtitle()`。
- 若为**自动生成字幕**(文件名含 `.auto.` 或 `-auto.`,例如 `sub.en-auto.vtt`)、无字幕(抖音/快手常见),或用户要精确逐字稿 → **不信自动字幕**,走第 3 步本地 ASR。
